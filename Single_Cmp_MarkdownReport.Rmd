...
suppressWarnings(suppressPackageStartupMessages(check.packages(c("tidyverse", "readxl", "investr", "broom", "kableExtra"))))
...
cSummary_table <- cSummary %>%
  mutate(dev = cell_spec(dev, "html", color = ifelse(abs(dev) > 20, "red", "black"))) %>%
  rename(`% Dev` = dev,
         Estimated.Conc = estimate,
         lower_est = lower,
         upper_est = upper) %>%
