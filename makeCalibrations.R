...
makeCalibrationModel <- function(dataset) {
  
  calCurve <- dplyr::filter(as.data.frame(dataset), Sample.Type == "Standard", is.na(Peak.Status) | Peak.Status != "Excluded")
  ...
  return = model.list[[which.model]]
  
  return(return)
}

getSamplePredictions <- function(dataset, calibration){
  
  calCurve.model <- calibration

    sample_measures <- dplyr::filter(as.data.frame(dataset), Sample.Type != "Blank") %>%
    select(Sample.Name,Sample.ID,Raw.Response,ISTD.Response) %>%
    mutate(Raw.Response = ifelse(is.na(Raw.Response), 0, Raw.Response)) %>%
    mutate(Area.Ratio = Raw.Response/ISTD.Response) 
  
  reverse.calibration <- function(subject, data) {
    measures <- dplyr::filter(data, Sample.ID == subject)$Area.Ratio
    
    calCurve <- model.frame(calCurve.model)# extract original calCurve from model for inverse estimates
  ...
  predictions <- map_df(.x = subjects, .f = reverse.calibration, data = sample_measures) %>%
